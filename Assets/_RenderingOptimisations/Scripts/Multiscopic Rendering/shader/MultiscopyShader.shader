Shader "MultiscopyShader" 
{	
	Properties {
		_ViewTex1 ("1st (RGB)", RECT) = "white" {}	// the available monitor has 8 views
	...
	SubShader {
		...
		{		
			...
			switch(viewIndex)    ///14725836 - the order of the views that are being displayed
			{
				case 0:  output = tex2D(_ViewTex6,float2(Xpixel * _multiplX,Ypixel *_multiplY));
				break;
				case 1:  output = tex2D(_ViewTex3,float2(Xpixel * _multiplX,Ypixel *_multiplY));
				break;
				case 2:  output = tex2D(_ViewTex8,float2(Xpixel * _multiplX,Ypixel *_multiplY));
				break;
				case 3:  output = tex2D(_ViewTex5,float2(Xpixel * _multiplX,Ypixel *_multiplY));
				break;
				case 4:  output = tex2D(_ViewTex2,float2(Xpixel * _multiplX,Ypixel *_multiplY));
				break;
				case 5:  output = tex2D(_ViewTex7,float2(Xpixel * _multiplX,Ypixel *_multiplY));
				break;
				case 6:  output = tex2D(_ViewTex4,float2(Xpixel * _multiplX,Ypixel *_multiplY));
				break;
				case 7:  output = tex2D(_ViewTex1,float2(Xpixel * _multiplX,Ypixel *_multiplY));
				break;
				case 8:  output = tex2D(_ViewTex6,float2(Xpixel * _multiplX,Ypixel *_multiplY)); //8==1, I just put this here in case....
				break; 
			}
			return output;
		}
		...
		{	
			fixed4 output;
			uint X = (int)(i.uv.x * _WidthRes);	// calculate the pixel coordinates of this texel
