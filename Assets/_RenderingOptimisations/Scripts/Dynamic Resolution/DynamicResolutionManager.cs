...
public class DynamicResolutionManager : MonoBehaviour
{
    ...
    void RecreateRendTex()
    {
        ...
        Destroy(rendTexture);       // if the old render texture is not explicitly destroyed there will be a memory leak that could lead to freeze and crash
        rendTexture = new RenderTexture((int)(Screen.width * resolutionMultiplierWidth), (int)(Screen.height * resolutionMultiplierHeight), 16, RenderTextureFormat.ARGB32);
        GetComponent<Camera>().targetTexture = rendTexture;
    }
    ...
    void OnGUI()
    {
        ...
        {
            Graphics.DrawTexture(new Rect(0, 0, Screen.width, Screen.height), rendTexture);
        }
