...
public class MultiscopyController_DynamicResolution : MultiscopyController_Base 
{
	...
	void RecreateRendTex()
	{
		...
		for (int i=0; i<8; i++)     //8 cameras for the alioscopy setup
		{
            Destroy(camRendTexture[i]);     // if the old render texture is not explicitly destroyed there will be a memory leak that could lead to freeze and crash
			camRendTexture[i] = new RenderTexture((int)(resolutionMultiplierWidth*(Screen.width*3)/8), (int)(resolutionMultiplierHeight * Screen.height/3), 24);
			camerasGO[i].gameObject.GetComponent<Camera>().targetTexture  = camRendTexture[i];
